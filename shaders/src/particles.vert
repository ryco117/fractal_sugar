...
void main() {
	...
	float t = fract((gl_VertexIndex) / particleCount);
	if(t < c1) {
		outColor = vec4(mix(vec3(0.2), vec3(endC1.x, endC1.y * t/c1, endC1.z), t / c1), 1.0);
	} else if(t < c2) {
		outColor = vec4(mix(endC1, endC2, (t - c1)/(c2 - c1)), 1.0);
	} else if(t < c3) {
		outColor = vec4(mix(endC2, endC3, (t - c2)/(c3 - c2)), 1.0);
	} else {
		outColor = vec4(mix(endC3, vec3(1.0, 0.0, 0.0), (t - c3)/(1.0 - c3)), 1.0);
	}
}